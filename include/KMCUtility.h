...
void On(std::function<void(const EventType *)> callback) {
    RE::ScriptEventSourceHolder::GetSingleton()->AddEventSink<EventType>(new CallbackEventSink<EventType>(callback));
}
...
enum class KMCInterruptType { 
    add_item,
    on_hit
};

enum class KMCDetectionType { 
    keyword,
    faction, 
    magic_effect_keyword,
    global
};
...
public:
    ...
    bool IsAllDisable() {
        if (Widget == false && Word == false && NamePlate == false && NamePlateName == false && Sound == false && SE == false) {
            return true;
        }
        return false;
    }
...
long long KMCFindVector(std::vector<std::pair<std::string, std::string>> *target, std::string key, long long defaultValue);
std::string KMCFindVector(std::vector<std::pair<std::string, std::string>> *target, std::string key, std::string defaultValue);
...
void NamePlateFadeOut(KMCNPLoadedWidget id, std::string aaaakmcroot);
    ...
    //std::vector<RE::TESObjectREFR *> FindAllReferencesWithKeyword(RE::TESObjectREFR *a_ref, RE::TESForm *a_formOrList,
